...
const register = async (req, res) => {
  ...
  if (user) {
    return res.status(500).json({ message: "Thehe is email already" });
  }
  ...
  const token = await jwt.sign({ id: newUser._id }, "SECRET_TOKEN", {
    expiresIn: "1h",
  });
  const cookieOptions = {
    httpOnly: true,
...
const login = async (req, res) => {
  ...
  const token = await jwt.sign({ id: user._id }, "SECRET_TOKEN", {
    expiresIn: "1h",
  });
  const cookieOptions = {
    httpOnly: true,
...
const forgatPassword = async (req, res) => {
  ...
  const message = `you can use this token for reset password : ${passwordUrl}`;
  try {
